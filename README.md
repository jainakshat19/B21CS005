...
## A20
XV6 boots by BIOS/UEFI initialization, executing the bootloader (GRUB), loading initial boot code, and transferring control to the kernel. The bootloader locates and loads XV6's kernel into memory from the file system. The kernel initializes essential structures, sets up device drivers, and transitions to user mode, creating the 'init' process. This process marks the system's readiness for user interaction, summarizing the steps from power-on to XV6 kernel execution.
